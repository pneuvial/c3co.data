...
baseDir <- Arguments$getWritablePath(file.path("geoData"))
filePaths <- getGEOSuppFiles(dataSet, baseDir=baseDir)

fl <- file.path(baseDir, dataSet, "filelist.txt")
library(dplyr)
...
untar(file.path(baseDir,dataSet, "GSE47077_raw.tar"), files=gzfiles, exdir=path)
